...
procesar() {
    ...
    mkdir -p "$GRUPO"/{raw,clean,suSCRample,reports,tmp}
    ...
    for SRR in "${SAMPLES[@]}"; do
        ...
        zcat "$GRUPO/clean/${SRR}_clean_R1.fastq.gz" | \
            ...
            gzip > "$GRUPO/suSCRample/${SRR}_sub_R1.fastq.gz"

        zcat "$GRUPO/clean/${SRR}_clean_R2.fastq.gz" | \
            seqtk sample -s100 - 0.3 | \
            gzip > "$GRUPO/suSCRample/${SRR}_sub_R2.fastq.gz"
        ...
        fastqc \
            --threads "$THREADS" \
            "$GRUPO/suSCRample/${SRR}_sub_R1.fastq.gz" \
            "$GRUPO/suSCRample/${SRR}_sub_R2.fastq.gz" \
            -o "$GRUPO/reports"
